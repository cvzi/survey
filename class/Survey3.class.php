...
class Survey3 {
    ...
    function getEverythingForVoting($uid) {
        ...
        foreach ($this->xml->set as $set) {
            ...
            $result[$name] = array('combination' => $combination, 'questions' => $mupltiple_question_array, 'a' => $a_array);
        }
...
FROM `%(stats)s` AS `t2`
WHERE `t2`.`for_id` = `t1`.`for_id`
AND `t2`.`set_id` = `t1`.`set_id` ) AS `set_number`
...
ORDER BY `for_id`,`set_id`,`set_number`
...
';
            ...
            foreach ($question_array as &$record) {
                ...
                if (isset($result_assoc[$for_id])) {
                    if (!isset($result_assoc[$for_id]['sets'][$a_text])) {
                        ...
                        ++$result_assoc[$for_id]['total'];
                    } else {
                        $result_assoc[$for_id]['sets'][$a_text] = $set_number;
                        ++$result_assoc[$for_id]['total'];
                    }
                } else {
                    $result_assoc[$for_id] = $record;
                    unset($result_assoc[$for_id]['a_text']);
                    unset($result_assoc[$for_id]['set_id']);
                    unset($result_assoc[$for_id]['set_number']);

                    $result_assoc[$for_id]['sets'] = array();
                    $result_assoc[$for_id]['sets'][$a_text] = $set_number;
                    $result_assoc[$for_id]['total'] = 1;
                }
                unset($record);
            }
            ...
            foreach ($result_assoc as &$record) {
                 ...
                 *
                 * Correct that:
                 * Gehrig und Eisele 13.21%
                 * Eisele und Gehrig 7.55%
                 *
                 */

                $keys = array_keys($record['sets']);

                foreach ($keys as &$oldkey) {
                    if (strpos($oldkey, ' und ') === false)
                        continue;

                    $key_ar = explode(' und ', $oldkey);
                    rsort($key_ar);
                    $newkey = implode(' und ', $key_ar);

                    if ($oldkey != $newkey) {
                        if ($record['sets'][$newkey]) {
                            $record['sets'][$newkey]['votes'] += $record['sets'][$oldkey];
                        } else {
                            $record['sets'][$newkey] = $record['sets'][$oldkey];
                        }
                        unset($record['sets'][$oldkey]);
                    }
                }
                unset($keys);
...
LEFT JOIN `user` ON `user`.`id` = `tables2`.`uid` ORDER BY `%s` %s', $middlePart, $this->mysql->escape($order_by), $this->mysql->escape($order_direction));
    ...
    function db_table_structure($type, $name, $combi_sets=false) {
